"""Windowed reporting over the store (D5) — `report` reads `fact_session`
only and never ingests. Resolves `--since`/`--from`/`--to`/`--today`/`--agent`
into a `[start, end)` date range, aggregates by `agent_type` (spawns, not
parent sessions), computes a prior-window delta, applies the low-volume
guard, and rolls spawns up per `parent_session_id` (the intra-session
parent lens).
"""
