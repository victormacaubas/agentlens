...
The system SHALL provide a per-parent-session rollup grouping spawns by `parent_session_id`, summarizing fan-out and health (spawn count, failures, denials) for that parent.

#### Scenario: Parent fan-out summarized

- **WHEN** a parent session fanned out four subagents of which one failed and one hit denials
- **THEN** the parent lens reports 4 spawns with 1 failure and 1 denial for that `parent_session_id`

### Requirement: Emit deterministic verdict-JSON slice

The system SHALL emit the deterministic slice of the verdict JSON — per-session counts and window rollups — with no LLM scores, and SHALL read exclusively from the store (never ingesting on read).

#### Scenario: Deterministic numbers without scores

- **WHEN** `report --since 7d` runs against a populated store
- **THEN** it emits per-session counts and window rollups and includes no LLM score fields
