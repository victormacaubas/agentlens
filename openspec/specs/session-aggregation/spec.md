...
Derives one `fact_session` row per spawn from parsed session data, combining event-derived tool counts with transcript-read usage, turn, and duration fields. Produces the deterministic grain that windowed reporting and the skill-usage bridge read from.
