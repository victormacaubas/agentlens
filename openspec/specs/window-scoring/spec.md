...
- **THEN** no further judge call is started, the run reports the ceiling as its stop
  reason, and it reports how many spawns remain unattempted
...
- **THEN** none of that reuse counts against the ceiling, and the run continues to
  the spawns that need scoring
